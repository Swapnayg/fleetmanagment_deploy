    ...
    fetch('https://secondsweb.com/inv_stock_data'+'/'+localStorage.getItem('id')).then((res) =>
      res.json().then((jsdata) => {
       for (let i = 0; i < jsdata.stock_data.length; i++) {
            ...
            {
              row += '<td style="text-transform:capitalize;"><span class="bg-danger-focus text-danger-main px-24 py-4 rounded-pill fw-medium text-sm" style="font-size: 12px !important;">'+jsdata.stock_data[i].stock_status.toString().trim()+'</span></td>';
            }
       ...
       for (let i = 0; i < jsdata.cust_data.length; i++) {
        ...
        {
          row += '<td style="text-transform:capitalize;"><span class="bg-danger-focus text-danger-main px-24 py-4 rounded-pill fw-medium text-sm" style="font-size: 12px !important;">'+jsdata.cust_data[i].stock_status.toString().trim()+'</span></td>';
        }
  ...
  useEffect(() => {
    ...
    $(document).off('click', '.stock_edit').on("click", '.stock_edit', function(e){
      ...
      setedithidden(false);
        ...
        var row_id = $(this).attr("data-lable");
        fetch('https://secondsweb.com/get_stk_return_data', { 
          method: 'POST', 
      ...
      $(document).off('click', '.stock_delete').on("click", '.stock_delete', function(e){
        ...
        var row_type = $(this).attr("data-type");
        setId(row_id);
        fetch('https://secondsweb.com/invoice_stock_delete/'+row_id+'/'+row_type+'/'+localStorage.getItem('id'), { 
          method: 'DELETE', 
  ...
  const handleSubmit = (e) => {
    ...
    if(canSubmit)
    {
      if($("#btnordersubmit").text().trim() == "Update")
      {
        ...
        fetch('https://secondsweb.com/update_stk_return_values', { 
          method: 'POST', 
          headers: {   'Accept': 'application/json',
            'Content-Type': 'application/json'  }, 
