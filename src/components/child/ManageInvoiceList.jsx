...
const ManageInvoiceList = () => {
  ...
  function refreshtable()
  {
    ...
    fetch('https://secondsweb.com/inv_invoice_data'+'/'+localStorage.getItem('id')).then((res) =>
      res.json().then((jsdata) => {
       for (let i = 0; i < jsdata.length; i++) {
        ...
        {
          row += '<td style="text-transform:capitalize;"><span class="bg-danger-focus text-danger-main px-24 py-4 rounded-pill fw-medium text-sm" style="font-size: 12px !important;">'+ jsdata[i].invoice_status.toString().trim()+'</span></td>';
        }
  ...
  useEffect(() => {
    ...
    $(document).off('click', '.invoice_edit').on("click", '.invoice_edit', function(e){
      ...
      setedithidden(false);
        ...
        var row_id = $(this).attr("data-lable");
        fetch('https://secondsweb.com/get_inv_invoice_data', { 
          method: 'POST', 
    ...
    $(document).off('click', '.invoice_delete').on("click", '.invoice_delete', function(e){
        ...
        var row_id = $(this).attr("data-lable");
        setId(row_id);
        fetch('https://secondsweb.com/invoice_inv_delete/'+row_id+'/'+localStorage.getItem('id'), { 
          method: 'DELETE', 
  ...
  const handleSubmit = (e) => {
    ...
    if(canSubmit)
    {
      if($("#btnordersubmit").text().trim() == "Update")
      {
        ...
        fetch('https://secondsweb.com/update_invoice_values', { 
          method: 'POST', 
          headers: {   'Accept': 'application/json',
            'Content-Type': 'application/json'  }, 
